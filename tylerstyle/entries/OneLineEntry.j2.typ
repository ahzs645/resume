...
{% if lowercase_section_title in ["certifications skills", "certifications_skills", "certifications & skills", "skills"] %}
{% if entry.label|trim %}*{{ entry.label|safe }}:* {% endif %}{{ entry.details|replace('\\(', '(')|replace('\\)', ')')|safe }}
