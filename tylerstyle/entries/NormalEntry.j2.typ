...
// Customized layout for Media Links section
  ...
  grid(
    columns: (1fr),
    align: (left),
    text("{{ entry.summary|replace('\\(', '(')|replace('\\)', ')') }}")
  )
  {% endif %}
...
// Wrap entire entry in entry_content to keep it together
  ...
  {% if entry.summary %}
  // Summary line (like institution/organization)
  grid(
    columns: (1fr, auto),
    align: (left, right),
  ...
  // Conditional rendering: bullets for awards, plain text for projects
    {% if is_project_like %}
      // Projects - indented bullets, with the "Technologies - " line
      // set flush as a lead-in rather than as a bullet.
      {% for highlight in entry.highlights %}
      {% set unescaped_highlight = unescape(highlight) %}
      {% set content = unescaped_highlight[2:] if unescaped_highlight[:2] == '- ' else unescaped_highlight %}
      {% if content[:15] == 'Technologies - ' %}
      [#box(width: 1em){{ content }}];
      {% else %}
      bullet_line([{{ content }}]);
      {% endif %}
